...
export function OperationForm({ entityId }: { entityId: string }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      console.log("ParentTool", parentTool);
      const pt = await GetEntityDetails("tool", parentId);
      console.log(pt);
      const stationId = pt.ParentID;
      console.log("StationID", stationId);
      setParentStation(await GetEntityDetails("station", stationId));
      console.log("ParentStation", parentStation);
      const ps = await GetEntityDetails("station", stationId);
      console.log(ps);
      setMeta({
              ...
              <FormItem>
                ...
                <Select
                  ...
                  onValueChange={(value) => {
                    ...
                    localStorage.setItem(entityId, JSON.stringify(json));
                    queryClient.invalidateQueries({
                      queryKey: ["station", parentStation.entityId],
                    });
                  }}
                >
                  <FormControl>
